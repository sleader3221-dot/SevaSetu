...
export default function PassbookModal({ isOpen, onClose, data, isLoading }: PassbookModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-white rounded-3xl shadow-2xl border border-orange-100 overflow-hidden my-8 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/70 print:hidden">
          ...
          <div className="flex items-center gap-3">
            <Button
              ...
              size="sm"
              onClick={handlePrint}
              className="border-gray-200 text-gray-700 hover:bg-white flex items-center gap-1.5"
            >
        ...
        <div id="passbook-print-area" className="p-6 sm:p-10 space-y-8 bg-white">
          
          {isLoading ? (
            <div className="py-20 text-center space-y-4">
              <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto"></div>
              <p className="text-gray-600 font-semibold text-lg">Cryptographically generating your Citizen Entitlement Passbook...</p>
              <p className="text-gray-400 text-sm">Aggregating real-time scheme ceilings via AWS Cloud Core</p>
            </div>
          ) : data ? (
            <>
              {/* Passbook Header */}
              <div className="border-b-2 border-orange-200/80 pb-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 rounded-2xl px-4 py-2.5 text-right">
                ...
                <div className="mt-6 p-4 rounded-2xl bg-orange-50/60 border border-orange-100 flex flex-wrap items-center justify-between gap-3 text-xs">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500 font-medium">Certificate Ref ID:</span>
                    <span className="font-mono font-bold text-gray-900 bg-white px-2.5 py-1 rounded-lg border border-orange-200">
                      {data.passbook_id}
                    </span>
                    <button
                      onClick={handleCopyId}
                      className="p-1 text-gray-400 hover:text-primary transition-colors print:hidden"
                  ...
                  <div className="text-gray-500">
                    Generated: <strong className="text-gray-800">{data.generated_at}</strong>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                    <span className="text-[11px] text-gray-500 block">State / UT</span>
                    <span className="font-bold text-gray-800 text-sm">{data.citizen_profile.state}</span>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                    <span className="text-[11px] text-gray-500 block">Occupation</span>
                    <span className="font-bold text-gray-800 text-sm">{data.citizen_profile.occupation}</span>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                    <span className="text-[11px] text-gray-500 block">Social Category</span>
                    <span className="font-bold text-gray-800 text-sm">{data.citizen_profile.category}</span>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                    <span className="text-[11px] text-gray-500 block">Annual Income</span>
                    <span className="font-bold text-gray-800 text-sm">₹{data.citizen_profile.annualIncome.toLocaleString('en-IN')}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-6 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs text-gray-500">
                <div className="flex items-center gap-3">
                  ...
                  <div className="w-16 h-16 bg-gray-100 p-1.5 rounded-lg border border-gray-300 flex items-center justify-center">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800">Digital Verification QR</p>
                    <p className="text-[11px] text-gray-400">Scan at CSC / Seva Kendra to verify validity</p>
                    <p className="text-[10px] text-emerald-600 font-bold mt-0.5">SHA256: {data.passbook_id}</p>
                  </div>
                </div>

                <div className="text-right sm:max-w-xs text-[11px] text-gray-400">
